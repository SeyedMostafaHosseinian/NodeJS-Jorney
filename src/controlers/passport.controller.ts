...
export class PassportController {
    ...
    constructor(public baseRoute: string) {
        this.passportRepository = AppDataSource.dataSource.getRepository(PassportEntity);
    ...
    private _addPassportListener() {
        app.post(`${this.baseRoute}/add`, async (req, res, next) => {
            ...
            if (existPassport) {
                res
                    .status(409)
                    .send('Passport for this user is existed');
                return;
            }
    ...
    private _updatePassportListener() {
        app.patch(`${this.baseRoute}`, async (req, res, next) => {
            ...
            if (!user) {
                res
                    .status(405)
                    .send('Not allowed change passport without user');
                return;
            }
